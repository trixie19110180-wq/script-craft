import express from "express";
import helmet from "helmet";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { adminPassword, adminUsername, attachUser, clearSession, createSession, hashPassword, isAdminUser, publicUser, requireUser, verifyPassword } from "./auth.js";
import { all, assetFromRow, dbBool, get, projectFromRow, projectSummary, run } from "./db.js";
import { importEntryEnt, importScratchSb3, importScriptCraft } from "./importers.js";
import { copyStoredAsset, removeAssetFile, storeAssetBuffer, uploadDir, uploadImage, uploadProjectFile } from "./uploads.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const rootDir = path.resolve(__dirname, "..");
const app = express();
const port = Number(process.env.PORT || 3000);

app.use(
  helmet({
    contentSecurityPolicy: false,
    crossOriginResourcePolicy: { policy: "same-site" }
  })
);
app.use(express.json({ limit: "1mb" }));
app.use(attachUser);
app.use("/uploads", express.static(uploadDir, { fallthrough: false, maxAge: "7d" }));

const defaultProjectData = {
  stage: {
    width: 480,
    height: 360,
    backgroundColor: "#eef3ff",
    backgroundAssetId: null,
    script: {
      language: "javascript",
      code: "function start() {\n  console.log(\"Background ready\");\n}\n\nfunction update(dt) {\n}"
    }
  },
  variables: [],
  sprites: [
    {
      id: "sprite-1",
      name: "Sprite 1",
      x: 240,
      y: 180,
      size: 72,
      rotation: 0,
      visible: true,
      costumeAssetId: null,
      shape: "logo",
      script: {
        language: "javascript",
        code: "function start() {\n  say(\"Hello from ScriptCraft\", 1800);\n}\n\nfunction update(dt) {\n  if (key(\"ArrowRight\")) changeX(4);\n  if (key(\"ArrowLeft\")) changeX(-4);\n  if (key(\"ArrowUp\")) changeY(4);\n  if (key(\"ArrowDown\")) changeY(-4);\n  bounceOnEdge();\n}"
      }
    }
  ]
};

function validateUsername(username = "") {
  return /^[A-Za-z0-9_]{3,24}$/.test(username);
}

function validateProjectAccess(project, user, res) {
  if (!project) {
    res.status(404).json({ error: "Project not found." });
    return false;
  }
  if (!project.published && (!user || (Number(user.id) !== Number(project.author_id) && !isAdminUser(user)))) {
    res.status(403).json({ error: "This project is private." });
    return false;
  }
  return true;
}

async function requireOwner(projectId, user, res) {
  const project = await get("SELECT * FROM projects WHERE id = ?", [projectId]);
  if (!project) {
    res.status(404).json({ error: "Project not found." });
    return null;
  }
  if (Number(project.author_id) !== Number(user.id) && !isAdminUser(user)) {
    res.status(403).json({ error: "You can only change your own projects." });
    return null;
  }
  return project;
}

async function loadProject(id, user) {
  const row = await get(
    `SELECT projects.*, users.username AS author, thumb.public_url AS thumbnail_url,
            original.title AS remix_of_title, original_author.username AS remix_of_author
     FROM projects
     JOIN users ON users.id = projects.author_id
     LEFT JOIN assets AS thumb ON thumb.id = projects.thumbnail_asset_id
     LEFT JOIN projects AS original ON original.id = projects.remix_of_project_id
     LEFT JOIN users AS original_author ON original_author.id = original.author_id
     WHERE projects.id = ?`,
    [id]
  );
  if (!validateProjectAccess(row, user, { status: () => ({ json: () => {} }) })) return null;
  const assets = (await all("SELECT * FROM assets WHERE project_id = ? ORDER BY created_at DESC", [id])).map(assetFromRow);
  return projectFromRow(row, assets);
}

app.get("/api/health", (_req, res) => {
  res.json({ ok: true, name: "ScriptCraft" });
});

app.get("/api/auth/me", (req, res) => {
  res.json({ user: req.user });
});

app.post("/api/auth/signup", async (req, res) => {
  const username = String(req.body.username || "").trim();
  const password = String(req.body.password || "");
  if (!validateUsername(username)) {
    res.status(400).json({ error: "Use 3-24 letters, numbers, or underscores for your username." });
    return;
  }
  if (password.length < 8) {
    res.status(400).json({ error: "Use a password with at least 8 characters." });
    return;
  }

  try {
    const passwordHash = await hashPassword(password);
    const result = await run("INSERT INTO users (username, password_hash) VALUES (?, ?) RETURNING id", [username, passwordHash]);
    await createSession(res, req, result.lastInsertRowid);
    const user = await get("SELECT id, username, created_at FROM users WHERE id = ?", [result.lastInsertRowid]);
    res.status(201).json({ user: publicUser(user) });
  } catch (error) {
    if (String(error.message).includes("UNIQUE") || String(error.message).includes("duplicate")) {
      res.status(409).json({ error: "That username is already taken." });
      return;
    }
    res.status(500).json({ error: "Could not create the account." });
  }
});

app.post("/api/auth/login", async (req, res) => {
  const username = String(req.body.username || "").trim();
  const password = String(req.body.password || "");
  const row = await get("SELECT * FROM users WHERE LOWER(username) = LOWER(?)", [username]);
  if (!row || !(await verifyPassword(password, row.password_hash))) {
    res.status(401).json({ error: "Username or password is incorrect." });
    return;
  }
  createSession(res, req, row.id);
  res.json({ user: publicUser(row) });
});

app.post("/api/auth/logout", async (req, res) => {
  await clearSession(req, res);
  res.json({ ok: true });
});

app.get("/api/projects", async (req, res) => {
  const mine = req.query.mine === "1";
  if (mine) {
    if (!req.user) {
      res.status(401).json({ error: "You need to log in first." });
      return;
    }
    const rows = isAdminUser(req.user)
      ? await all(
          `SELECT projects.*, users.username AS author, thumb.public_url AS thumbnail_url
           FROM projects
           JOIN users ON users.id = projects.author_id
           LEFT JOIN assets AS thumb ON thumb.id = projects.thumbnail_asset_id
           ORDER BY projects.updated_at DESC`
        )
      : await all(
          `SELECT projects.*, users.username AS author, thumb.public_url AS thumbnail_url
           FROM projects
           JOIN users ON users.id = projects.author_id
           LEFT JOIN assets AS thumb ON thumb.id = projects.thumbnail_asset_id
           WHERE projects.author_id = ?
           ORDER BY projects.updated_at DESC`,
          [req.user.id]
        );
    res.json({ projects: rows.map(projectSummary) });
    return;
  }

  const rows = await all(
    `SELECT projects.*, users.username AS author, thumb.public_url AS thumbnail_url
     FROM projects
     JOIN users ON users.id = projects.author_id
     LEFT JOIN assets AS thumb ON thumb.id = projects.thumbnail_asset_id
     WHERE projects.published = ?
     ORDER BY projects.updated_at DESC`,
    [dbBool(true)]
  );
  res.json({ projects: rows.map(projectSummary) });
});

app.post("/api/projects", requireUser, async (req, res) => {
  const title = String(req.body.title || "Untitled Project").trim().slice(0, 80) || "Untitled Project";
  const description = String(req.body.description || "").trim().slice(0, 600);
  const result = await run("INSERT INTO projects (author_id, title, description, data_json) VALUES (?, ?, ?, ?) RETURNING id", [
    req.user.id,
    title,
    description,
    JSON.stringify(defaultProjectData)
  ]);
  res.status(201).json({ project: await loadProject(result.lastInsertRowid, req.user) });
});

app.post("/api/projects/import", requireUser, uploadProjectFile.single("project"), async (req, res) => {
  if (!req.file) {
    res.status(400).json({ error: "Choose a Scratch, Entry, or ScriptCraft project file." });
    return;
  }

  try {
    const filename = req.file.originalname || "Imported Project";
    const lower = filename.toLowerCase();
    let projectId;
    if (lower.endsWith(".sb3")) {
      projectId = await importScratchSb3(req.user.id, req.file.buffer, filename);
    } else if (lower.endsWith(".ent")) {
      projectId = await importEntryEnt(req.user.id, req.file.buffer, filename);
    } else if (lower.endsWith(".scriptcraft") || lower.endsWith(".json")) {
      projectId = await importScriptCraft(req.user.id, req.file.buffer.toString("utf8"));
    } else {
      res.status(400).json({ error: "Supported files: .sb3, .ent, .scriptcraft, and .json." });
      return;
    }
    res.status(201).json({ project: await loadProject(projectId, req.user) });
  } catch (error) {
    res.status(400).json({ error: error.message || "Could not import that project." });
  }
});

app.post("/api/projects/:id/remix", requireUser, async (req, res) => {
  const original = await get("SELECT * FROM projects WHERE id = ?", [req.params.id]);
  if (!validateProjectAccess(original, req.user, res)) return;
  const originalAssets = await all("SELECT * FROM assets WHERE project_id = ?", [original.id]);
  const data = JSON.parse(original.data_json);
  const result = await run(
    "INSERT INTO projects (author_id, title, description, data_json, published, remix_of_project_id) VALUES (?, ?, ?, ?, ?, ?) RETURNING id",
    [req.user.id, `Remix of ${original.title}`.slice(0, 80), original.description, JSON.stringify(data), dbBool(false), original.id]
  );
  const remixId = result.lastInsertRowid;
  const assetIdMap = new Map();

  for (const asset of originalAssets) {
    const copied = await copyStoredAsset(asset);
    if (!copied) continue;
    const inserted = await run(
      "INSERT INTO assets (project_id, kind, name, mime_type, file_path, public_url) VALUES (?, ?, ?, ?, ?, ?) RETURNING id",
      [remixId, asset.kind, asset.name, asset.mime_type, copied.filePath, copied.publicUrl]
    );
    assetIdMap.set(asset.id, inserted.lastInsertRowid);
  }

  if (data.stage?.backgroundAssetId) {
    data.stage.backgroundAssetId = assetIdMap.get(data.stage.backgroundAssetId) || null;
  }
  for (const sprite of data.sprites || []) {
    if (sprite.costumeAssetId) sprite.costumeAssetId = assetIdMap.get(sprite.costumeAssetId) || null;
  }
  const thumbnailAssetId = original.thumbnail_asset_id ? assetIdMap.get(original.thumbnail_asset_id) || null : null;
  await run("UPDATE projects SET data_json = ?, thumbnail_asset_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [JSON.stringify(data), thumbnailAssetId, remixId]);
  res.status(201).json({ project: await loadProject(remixId, req.user) });
});

app.get("/api/projects/:id", async (req, res) => {
  const row = await get(
    `SELECT projects.*, users.username AS author, thumb.public_url AS thumbnail_url,
            original.title AS remix_of_title, original_author.username AS remix_of_author
     FROM projects
     JOIN users ON users.id = projects.author_id
     LEFT JOIN assets AS thumb ON thumb.id = projects.thumbnail_asset_id
     LEFT JOIN projects AS original ON original.id = projects.remix_of_project_id
     LEFT JOIN users AS original_author ON original_author.id = original.author_id
     WHERE projects.id = ?`,
    [req.params.id]
  );
  if (!validateProjectAccess(row, req.user, res)) return;
  const assets = (await all("SELECT * FROM assets WHERE project_id = ? ORDER BY created_at DESC", [req.params.id])).map(assetFromRow);
  res.json({ project: projectFromRow(row, assets) });
});

app.put("/api/projects/:id", requireUser, async (req, res) => {
  const project = await requireOwner(req.params.id, req.user, res);
  if (!project) return;
  const title = String(req.body.title || project.title).trim().slice(0, 80) || project.title;
  const description = String(req.body.description ?? project.description).trim().slice(0, 600);
  const data = req.body.data || JSON.parse(project.data_json);
  const published = req.body.published === undefined ? project.published : dbBool(req.body.published);
  const thumbnailAssetId = req.body.thumbnailAssetId === undefined ? project.thumbnail_asset_id : req.body.thumbnailAssetId || null;

  if (thumbnailAssetId) {
    const asset = await get("SELECT id FROM assets WHERE id = ? AND project_id = ?", [thumbnailAssetId, project.id]);
    if (!asset) {
      res.status(400).json({ error: "Thumbnail asset does not belong to this project." });
      return;
    }
  }

  await run(
    `UPDATE projects
     SET title = ?, description = ?, data_json = ?, published = ?, thumbnail_asset_id = ?, updated_at = CURRENT_TIMESTAMP
     WHERE id = ?`,
    [title, description, JSON.stringify(data), published, thumbnailAssetId, project.id]
  );
  res.json({ project: await loadProject(project.id, req.user) });
});

app.delete("/api/projects/:id", requireUser, async (req, res) => {
  const project = await requireOwner(req.params.id, req.user, res);
  if (!project) return;
  const assets = await all("SELECT file_path FROM assets WHERE project_id = ?", [project.id]);
  await run("DELETE FROM projects WHERE id = ?", [project.id]);
  await Promise.all(assets.map((asset) => removeAssetFile(asset.file_path)));
  res.json({ ok: true });
});

app.post("/api/projects/:id/assets", requireUser, uploadImage.single("image"), async (req, res) => {
  const project = await requireOwner(req.params.id, req.user, res);
  if (!project) {
    return;
  }
  if (!req.file) {
    res.status(400).json({ error: "Choose an image to upload." });
    return;
  }
  const kind = String(req.body.kind || "costume");
  if (!["sprite", "costume", "background", "thumbnail"].includes(kind)) {
    res.status(400).json({ error: "Asset kind must be sprite, costume, background, or thumbnail." });
    return;
  }
  const stored = await storeAssetBuffer(req.file.buffer, {
    originalName: req.file.originalname,
    mimeType: req.file.mimetype,
    folder: `scriptcraft/projects/${project.id}`
  });
  const result = await run(
    "INSERT INTO assets (project_id, kind, name, mime_type, file_path, public_url) VALUES (?, ?, ?, ?, ?, ?) RETURNING id",
    [project.id, kind, String(req.body.name || req.file.originalname).slice(0, 120), req.file.mimetype, stored.filePath, stored.publicUrl]
  );

  if (kind === "thumbnail") {
    await run("UPDATE projects SET thumbnail_asset_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [result.lastInsertRowid, project.id]);
  }
  const asset = await get("SELECT * FROM assets WHERE id = ?", [result.lastInsertRowid]);
  res.status(201).json({ asset: assetFromRow(asset), project: await loadProject(project.id, req.user) });
});

app.delete("/api/projects/:projectId/assets/:assetId", requireUser, async (req, res) => {
  const project = await requireOwner(req.params.projectId, req.user, res);
  if (!project) return;
  const asset = await get("SELECT * FROM assets WHERE id = ? AND project_id = ?", [req.params.assetId, project.id]);
  if (!asset) {
    res.status(404).json({ error: "Asset not found." });
    return;
  }
  await run("DELETE FROM assets WHERE id = ?", [asset.id]);
  await removeAssetFile(asset.file_path);
  res.json({ ok: true });
});

const distDir = path.join(rootDir, "client", "dist");
app.use(express.static(distDir));
app.get("*", (_req, res) => {
  res.sendFile(path.join(distDir, "index.html"));
});

app.use((error, _req, res, _next) => {
  res.status(400).json({ error: error.message || "Something went wrong." });
});

async function ensureAdminAccount() {
  const passwordHash = await hashPassword(adminPassword);
  const existing = await get("SELECT id FROM users WHERE LOWER(username) = LOWER(?)", [adminUsername]);
  if (existing) {
    await run("UPDATE users SET username = ?, password_hash = ? WHERE id = ?", [adminUsername, passwordHash, existing.id]);
    return;
  }
  await run("INSERT INTO users (username, password_hash) VALUES (?, ?)", [adminUsername, passwordHash]);
}

await ensureAdminAccount();

app.listen(port, () => {
  console.log(`ScriptCraft is running on port ${port}`);
});
